...
<template>
  <header class="osiyan-header">
    <div class="osiyan-header-inner">
      <button class="menu-toggle" type="button" aria-label="Toggle menu" :aria-expanded="menuOpen" @click="menuOpen = !menuOpen">
        <span /><span /><span />
      </button>
    ...
    <nav v-if="menuOpen" class="mobile-navigation" aria-label="Mobile site navigation">
      <RouterLink v-for="item in links" :key="item.label" :to="item.to">{{ item.label }}</RouterLink>
...
.osiyan-nav { position: absolute; left: 50%; top: 0; height: 86px; display: flex; align-items: center; gap: clamp(22px, 2.5vw, 42px); transform: translateX(-50%); white-space: nowrap; }
.osiyan-nav a { position: relative; color: #777; text-decoration: none; font-size: 18px; line-height: 1; font-weight: 400; transition: color .2s ease; }
...
.account-trigger { height: 56px; min-width: 70px; display: flex; align-items: center; justify-content: space-between; gap: 9px; border: 1px solid transparent; border-radius: 7px; padding: 3px 7px 3px 3px; color: #777; background: transparent; cursor: pointer; transition: border-color .2s ease, box-shadow .2s ease; }
...
.account-avatar { width: 48px; height: 48px; display: grid; flex: 0 0 48px; place-items: center; border-radius: 50%; color: #fff; background: #111; font-size: 18px; font-weight: 600; line-height: 1; }
...
.account-email { margin-top: 6px; color: #aaa; font-size: 16px; font-weight: 400; line-height: 1.35; }
...
@media (max-width: 800px) {
  ...
  .menu-toggle { position: absolute; right: 139px; top: 31px; display: block; width: 25px; border: 0; padding: 0; background: none; }
  ...
  .account-trigger { min-width: 47px; height: 50px; padding: 3px; }
  .account-avatar { width: 42px; height: 42px; flex-basis: 42px; font-size: 16px; }
  .account-chevron { display: none; }
  .account-dropdown { position: fixed; top: 83px; left: 14px; right: 14px; width: auto; }
  .mobile-navigation { position: absolute; top: 86px; left: 0; right: 0; min-height: calc(100vh - 86px); display: flex; flex-direction: column; padding: 34px 26px; background: #fff; }
  .mobile-navigation a { padding: 15px 0; border-bottom: 1px solid #dedede; color: #777; text-decoration: none; font-size: 24px; }
}
</style>
